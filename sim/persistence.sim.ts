...
/* ---- half-decoded messages resume where they were ---- */
{
  ...
  const before: Message[] = [
    ...
    { id: 'm2', mine: false, symbols, at: 2, puzzle: EMPTY_PUZZLE, echo, delivery: 'none' },
  ];
...
/* ---- a long history is capped without losing the newest ---- */
{
  const many: Message[] = Array.from({ length: MAX_STORED_MESSAGES + 75 }, (_, index) => ({
    ...
    puzzle: EMPTY_PUZZLE, echo: ECHO_START, delivery: 'delivered' as const,
  }));
...
/* ---- delivery state survives sensibly ---- */
{
  const stored = parseMessages(JSON.stringify([
    { id: 'a', mine: true, symbols: '...', at: 1, delivery: 'delivered' },
    { id: 'b', mine: true, symbols: '...', at: 2, delivery: 'sending' },
    { id: 'c', mine: true, symbols: '...', at: 3, delivery: 'queued' },
    { id: 'd', mine: true, symbols: '...', at: 4, delivery: 'nobody' },
    { id: 'e', mine: false, symbols: '...', at: 5 },
  ]));
  const by = (id: string) => stored.find((m) => m.id === id)!;
  check(by('a').delivery === 'delivered', 'a confirmed delivery was forgotten');
  check(by('d').delivery === 'nobody', 'a known-undelivered message was forgotten');
